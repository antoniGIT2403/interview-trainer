...
export class QuizzAngularComponent implements OnInit {
  ...
  ngOnInit(): void {
   this.listQuestionAngular =  this.questionsService.getListQuestionAngular();
  ...
  correction(){
    ...
    if( !this.listQuestionAngular[this.index].reponses.find( (vraiReponse) => vraiReponse === reponse) ) {
  ...
  answerSelection(option:string){
    if(  this.listQuestionAngular[this.index].reponseUser?.find( (reponse) => reponse === option )    ){
     let indexTodelete =  this.listQuestionAngular[this.index].reponseUser?.indexOf(option);
  ...
  renderSelectedCss(option:string) {
    ...
    if (this.listQuestionAngular[this.index].reponseUser && this.listQuestionAngular[this.index].reponseUser.find( (reponse) => reponse === option)){
      isSelected = true;
    }
    return {
